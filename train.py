...
# Create custom BERT layer in keras
class BertLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.bert = hub.Module("https://tfhub.dev/tensorflow/bert_en_cased_L-24_H-1024_A-16/1",
    ...
    def calls(self, inputs):
        inputs = [tf.keras.backend.cast(x, type="int32") for x in inputs]
        input_ids, input_mask, segment_ids = inputs
        bert_inputs = dict(
            input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids
        )
        result = self.bert(input=bert_inputs, signature="tokens", as_dict=True)
        return result
